...
with open('src/create_structure_models_with_predicted_zn/score_to_prob_calibrated_dict.json', 'r') as file:
    threshold_prob_dict = json.load(file)


def get_prob(score):
    for score_thresold in sorted(threshold_prob_dict.keys(), key=float):
        if score <= float(score_thresold):
            prob = float(threshold_prob_dict[score_thresold])
            return prob
    return float(
        min(threshold_prob_dict.values()))  # Default to higher than highest threshold the precision is lowest
...
def create_pymol_session_structure_with_predicted_zn(path_structure, list_of_predicted_ZN, path_output):
    ...
    if list_of_predicted_ZN is not None:
        ...
        for index, score_coordinates in enumerate(list_of_predicted_ZN):
            ...
            prob = get_prob(score)
            cmd.alter(selection_name, f"b={prob}")
        ...
        thresholds = list(map(float, threshold_prob_dict.values()))

        # Create a spectrum using these custom colors
        cmd.spectrum("b", "red_yellow_green", "predicted_zincs", minimum=min(thresholds),

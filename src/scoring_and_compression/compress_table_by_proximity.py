...
def table_compression(conn):
    ...
    for pdbid, coords_score_id in pdbid_data.items():
       whether_was_group_bigger_than_1= True # Initialize default to True before start the whole process of- 1) grouping to clusters of predicted coords with maximal distance of 2 angstrom between coords, and 2) select minimal scored coord from each group. Verify if it is still possible to create a group in which the maximum distance between any predicted coordinates is 2 Å.

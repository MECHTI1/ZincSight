...
def fetch_and_process_data(conn):
    ...
    for i in range(0, len(unique_match_ids), unique_match_ids_batch_size):
      ...
      #   data_dict.clear()
        ...
        dict_match_id_to_dis_CoordinationAngles_HISangles_stats_single_batch={}
        # Calculate Coordination Angles and HIS angles stats for each match_id
        for match_id, dict_residues_in_candidate_motif in data_dict.items():
            dict_match_id_to_dis_CoordinationAngles_HISangles_stats_single_batch[match_id] = caclulate_dis_CoordinationAngles_HISangles_stats(dict_residues_in_candidate_motif)
